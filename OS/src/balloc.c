...
static void balloc_parse_mmap(const struct mboot_info *info)
{
	...
	init_ram(info);
	const uintptr_t ram_begin = (uintptr_t)ram_fs_begin;
	const uintptr_t ram_end = (uintptr_t)ram_fs_end;

	__balloc_remove_range(&free_ranges, kbegin, kend);
	__balloc_remove_range(&free_ranges, ram_begin, ram_end);
}
