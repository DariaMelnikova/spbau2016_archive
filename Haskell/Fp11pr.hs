...
instance Monad m => Functor (LoggT m) where
  fmap  f x = LoggT $ do
            Logged s a <- (runLoggT x)
            return $ Logged s (f a)    
instance Monad m => Applicative (LoggT m) where
  pure a = LoggT $ pure (Logged "" a)
  f <*> a = LoggT $ do
            Logged fs fm <- (runLoggT f)
            Logged as am <- (runLoggT a)
            return $ Logged (as ++ fs) (fm am)
...
instance MonadTrans LoggT where
  lift m = LoggT $ do { a <- m; return (Logged "" a)}
